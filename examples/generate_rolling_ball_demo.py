...
SWELLS = (
    ((180, 150), 300.0, 900.0),  # (centre xy, sigma, peak)
...
LEVEL_NAMES = tuple(name for name, _g, _s, _w in DEMO_ROIS if name.startswith("level_"))
...
def _background(frame_index):
    ...
    growth = 1.0 + (SWELL_GROWTH - 1.0) * (frame_index / max(FRAME_COUNT - 1, 1))
    background = BACKGROUND_FLOOR + GRADIENT_PER_PX * x
    for (cx, cy), sigma, peak in SWELLS:
        background += (
            peak
            * growth
            * np.exp(-(((x - cx) ** 2 + (y - cy) ** 2) / (2.0 * sigma**2)))
        )
    return background
...
def _report(session, paths):
    ...
    for name, _geometry, _signal, what in DEMO_ROIS:
        print(
            f"{name:14s} {raw[name][0]:10.0f} {raw[name][-1]:10.0f} "
    ...
    print(f"  raw, they span      {spread_raw:8.0f} counts (the background talking)")
    print(
        f"  ball-corrected      {spread:8.0f} counts spread, "
        f"worst error {error:.0f} from the true {LEVEL_SIGNAL:.0f}"
    )
    if spread > LEVEL_TOLERANCE or error > LEVEL_TOLERANCE:
        print(f"  FAILED: expected agreement within {LEVEL_TOLERANCE:.0f} counts")
        return False
    print(f"  OK: within {LEVEL_TOLERANCE:.0f} counts of each other and of the truth")

    print(
        "  (the ball leaves a residue where the background "
        "curves sharply relative to its radius;"
    )
    print("   these swells are broad, so it is small)")

    small = _stats_by_name(session, paths, 20)
    print("\nover-rolling check — big_blob (r=45) measured with a ball of:")
    print(
        f"  r={DEMO_BALL_RADIUS} (clears it) {corrected['big_blob'][0]:8.0f}"
...
def _stats_by_name(session, paths, ball_radius):
    ...
    for path in paths:
        ...
        for roi in session.rois:
            by_name.setdefault(roi.name, []).append(result["stats"][roi.roi_id]["mean"])
    return by_name
...
def main():
    ...
    parser.add_argument(
        "output", nargs="?", default=".", help="where to create rolling_ball_demo/"
    )
    arguments = parser.parse_args()
    experiment_dir = Path(arguments.output).resolve() / "rolling_ball_demo"
    captures_dir = experiment_dir / "captures"
    paths = _write_frames(captures_dir, time.time() - FRAME_COUNT * FRAME_INTERVAL_S)
    print(
        f"wrote {len(paths)} frames of "
    ...
    save_roi_stats(experiment_dir, _stats(session, discovered, DEMO_BALL_RADIUS))
    print(f"session written with the ball ON at r={DEMO_BALL_RADIUS}px")
