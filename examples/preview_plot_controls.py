...
def section(flag, label, *content):
    ...
    return VGroup(
        HGroup(UItem(f"panel.{flag}", editor=IconToggleEditor()), Label(label)),
...
def _spin(
    ...
    return Item(
        ...
        width=width,
        editor=RangeEditor(
            low=bounds[0], high=bounds[1], mode="spinner", auto_set=True
        ),
...
def _toggle(name, label, enabled_when="", tooltip=""):
    return UItem(
        name,
        editor=InPlaceToggleEditor(on_label=label, off_label=label),
        enabled_when=enabled_when,
        tooltip=tooltip,
    )
    ...
    return HGroup(
        ...
        UItem(
            ...
            editor=IconButtonEditor(
                glyph=ICON_SAVE,
                tooltip="Save the plot at the Export tab's DPI and format",
            ),
        ),
    )
...
def axes_tab():
    ...
    return VGroup(
        VGrid(
            ...
            _axis_spin(
                "figure.x_min", "x_auto"
            ),  # Ensure these return items with fixed widths too!
        ...
        section(
            ...
            HGroup(
                ...
                UItem(
                    ...
                    editor=IconButtonEditor(
                        glyph=ICON_FUNCTION, tooltip="Fitted parameters per ROI"
                    ),
                ),
            ),
        ),
...
def cleanup_tab():
    return VGroup(
        ...
        HGroup(
            Item("figure.smooth_method", label="Smooth", width=DROPDOWN_W),
            _spin(
...
def build_view():
    return View(
        VGroup(
            top_row(),
            Tabbed(axes_tab(), fit_tab(), cleanup_tab(), export_tab()),
        ),
        title="ROI Intensities controls — tabbed",
        resizable=True,
    )
...
def main():
    ...
    parser.add_argument(
        "--shots", metavar="DIR", help="render PNGs offscreen instead of showing"
    )
