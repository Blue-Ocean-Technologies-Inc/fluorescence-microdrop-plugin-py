...
THREAD_APPLIED_SETTINGS = frozenset(ADVANCED_CONTROL_TYPES) | frozenset(
    ROI_SETTING_NAMES
)
...
def display_adjust_lut(gamma: float, contrast: float, brightness: float) -> np.ndarray:
...
def raw_to_qimage(img: np.ndarray) -> QImage:
    ...
    if frame.dtype == np.uint16 and frame.ndim == 2:
        qimage = QImage(
            frame.data, width, height, frame.strides[0], QImage.Format_Grayscale16
        )
        return qimage.copy()
    if frame.dtype == np.uint16 and frame.ndim == 3:
        ...
        rgba[:, :, :3] = frame
        rgba[:, :, 3] = 65535
        qimage = QImage(rgba.data, width, height, rgba.strides[0], QImage.Format_RGBA64)
        return qimage.copy()
...
def frame_to_qimage(img: np.ndarray) -> QImage:
    ...
    if frame.ndim == 2:
        qimage = QImage(
            frame.data, width, height, frame.strides[0], QImage.Format_Grayscale8
        )
    else:
        qimage = QImage(
...
class ASIVideoThread(QThread):
    ...
    def __init__(self, sdk_dir, camera_index, exposure=20000, gain=300, advanced=None):
        super().__init__()
        ...
        self._roi_settings = {
            name: self._pending_advanced.get(name) for name in ROI_SETTING_NAMES
        }
    ...
    def _apply_pending_advanced(self):
        ...
        if any(pending.get(name) is not None for name in ROI_SETTING_NAMES):
            ...
            self.camera.set_roi(
                ...
                img_type=ASI_IMG_TYPES[self._roi_settings["image_type"] or "raw16"],
                width=width,
                height=height,
            )
            ...
            img_type_names = {value: name for name, value in ASI_IMG_TYPES.items()}
            self._roi_settings["binning"] = self.camera.roi_binning
            self._roi_settings["image_type"] = img_type_names[self.camera.roi_img_type]
    ...
    def _auto_adjust(self, img):
        ...
        if self.auto_exposure:
            new_exposure = int(
                min(
                    max(self.exposure * ratio, ASI_EXPOSURE_MIN), self.auto_max_exposure
                )
            )
        ...
        if self.auto_gain:
            ...
            new_gain = int(min(max(self.gain + step, ASI_GAIN_MIN), self.auto_max_gain))
