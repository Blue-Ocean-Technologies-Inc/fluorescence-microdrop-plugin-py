...
from ..cameras.consts import (
    ...
    AUTO_MAX_EXPOSURE_UNIT_HIGHS,
    AUTO_MAX_EXPOSURE_UNITS,
...
ADVANCED_PANE_PERSISTED_TRAITS = ADVANCED_CAMERA_TRAITS + (
    "auto_max_exposure_value",
    "auto_max_exposure_unit",
)
...
class AdvancedCameraModel(HasTraits):
    ...
    image_type = Enum(
        *ASI_IMAGE_TYPE_CHOICES, desc="output image type (bit depth / color mode)"
    )
    resolution = Str(
        "full", desc="capture resolution: the full binned frame or a centered crop"
    )
    ...
    display_gamma = Range(
        ...
        DISPLAY_GAMMA_DEFAULT,
        mode="slider",
        desc="preview gamma curve (1.0 = neutral)",
    )
    ...
    def _reset_display_adjustments(self, event):
        self.trait_set(
            display_gamma=DISPLAY_GAMMA_DEFAULT,
    ...
    initial_temperature_text = Property(Str, observe="temperature_history.items")
    current_temperature_text = Property(Str, observe="temperature_history.items")
    ...
    def _record_temperature(self, event):
        ...
        self.temperature_history.append((now - self._monitoring_started, event.new))
    ...
    hardware_bin = Bool(False, desc="Bin on the sensor instead of in software")
    mono_bin = Bool(False, desc="Mono binning (less grid artifact on color cameras)")
    ...
    image_type_choices = Property(observe="camera_supported_image_types.items")
    ...
    def _get_binning_choices(self):
        ...
        return {
            value: label
            for value, label in ASI_BINNING_LABELS.items()
            if value in supported
        }

    def _get_image_type_choices(self):
        supported = self.camera_supported_image_types or list(ASI_IMAGE_TYPE_CHOICES)
        return {
            value: label
    ...
    @observe("camera_supported_bins.items,camera_supported_image_types.items")
    def _snap_to_supported(self, event):
        ...
        if (
            self.camera_supported_bins
            and self.binning not in self.camera_supported_bins
        ):
            self.binning = 1
        if (
            self.camera_supported_image_types
